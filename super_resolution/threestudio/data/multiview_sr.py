...
class MultiviewDataset(Dataset):
    def __init__(self, cfg: Any,mode,split = 'train',shuffle=False) -> None:
        ...
        if 'w' in frames[0]:
            assert frames[0]["w"] == frames[0]["h"]
            if mode == 'low_res':
                scale = frames[0]["w"] / self.cfg.low_resolution
            else:
                scale = frames[0]["w"] / self.cfg.high_resolution

            self.frame_w = round(frames[0]["w"] // scale)
            self.frame_h = round(frames[0]["h"] // scale)
        else:
            frame_path = os.path.join(self.cfg.dataroot, frames[0]["file_path"])
        ...
        for idx, frame in tqdm(enumerate(frames)):
            ...
            if (mode == 'low_res') or self.cfg.load_high_res_gt: #only load images in low res
                ...
                img = cv2.resize(img, (self.frame_w*self.cfg.latent_scale, self.frame_h*self.cfg.latent_scale), interpolation=cv2.INTER_AREA) #TODO: resize mode better be area
...
@register("multiview-camera-sr-datamodule")
