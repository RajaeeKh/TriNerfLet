...
from threestudio.models.triplaneencoder.triplane_encoder import (TriPlaneVolume,KPlaneVolume,MultiscaleKPlaneVolume,
                                                                MultiscaleKPlaneMulVolume
                                                                 )
...
def get_encoding(n_input_dims: int, config) -> nn.Module:
    ...
    elif config.otype == 'triplane_wavelet':
        encoding = TriPlaneVolume(
            number_of_features=config.triplane_channels,
            plane_resolution=config.triplane_resolution,
            init_sigma=0.1,
            lbound=1,
            viewdir_plane_resolution=-1,
            apply_activation_on_features=False,
            inner_multi_res_scale=config.triplane_wavelet_levels,
            inner_multi_res_scale_current=1,
            low_res_scale=config.low_res_scale,
            high_res_scale=config.high_res_scale,
            wavelet_type = config.wavelet_type,
            wavelet_base_resolution = config.wavelet_base_resolution
        )
    elif config.otype == 'k_planes':
        encoding = KPlaneVolume(base_resolution = config.base_resolution,
                                levels = config.levels,
                                channels = config.channels,
                                features_mode = config.features_mode
                                )
    elif config.otype == 'multiscale_k_planes':
        encoding = MultiscaleKPlaneVolume(base_resolution = config.base_resolution,
                                          low_res_levels = config.low_res_levels,
                                          high_res_levels = config.high_res_levels,
                                          channels = config.channels,
                                          features_mode = config.features_mode
                                          )
    elif config.otype == 'multiscale_k_planes_mul':
        encoding = MultiscaleKPlaneMulVolume(base_resolution = config.base_resolution,
                                          low_res_levels = config.low_res_levels,
                                          high_res_levels = config.high_res_levels,
                                          channels = config.channels,
                                          features_mode = config.features_mode
