...
@threestudio.register("deep-floyd-guidance4")
class DeepFloydGuidance(BaseObject):
    ...
    def configure(self) -> None:
        ...
        # self.scheduler.set_timesteps(self.cfg.num_inference_steps, device=self.device) #TODO
    ...
    def generate_image(self,
        ...
        for i, t in enumerate(tqdm(timesteps)):
            ...
            steps_num += 1
            if steps_num == 5:
                latents_noisy = pred_original_sample  # TODO
                break
        ...
        **kwargs,
        ...
        if cache_data is None:
            ...
            if prompt_utils.use_perp_neg:
                ...
                with torch.no_grad():
                    res_img, aux = self.generate_image(
                        rgb=latents,
            ...
            res_img = cache_data['res_img']

        pred_original_sample = 2*res_img - 1
        ...
        # res = self.scheduler.step(noise_pred, t[0], latents_noisy)
        # latents_noisy = res.prev_sample
        # pred_original_sample = res.pred_original_sample
        pred_original_sample = pred_original_sample.clamp(-1,1)
        # w = w * (self.alphas[t].sqrt()/(1-self.alphas[t]).sqrt()) #TODO
        grad = w * ( latents - pred_original_sample)
        grad = torch.nan_to_num(grad)
        target = (latents - grad).detach()
        loss_sds = 0.5 * F.mse_loss(latents, target, reduction="sum") / batch_size
...
def test2():
    ...
    cfg_f = 'configs/dreamfusion-if2.yaml'
    text = ''
    targer_res = 64
    device = 'cuda:0'
    ...
    alphas = guidance.scheduler.alphas_cumprod
    w1 = (1 - alphas)
    plt.plot(w1)
    plt.show()

    w2 = alphas.sqrt() / (1 - alphas).sqrt()
    plt.plot(w2)
    plt.show()
