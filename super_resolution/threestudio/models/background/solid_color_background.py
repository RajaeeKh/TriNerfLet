...
class SolidColorBackground(BaseBackground):
    ...
    def forward(self, dirs: Float[Tensor, "B H W 3"]) -> Float[Tensor, "B H W Nc"]:
        # print(torch.ones(*dirs.shape[:-1], self.cfg.n_output_dims).device)
        # print(torch.ones(*dirs.shape[:-1], self.cfg.n_output_dims).dtype)
        # print(dirs.device)
        # print(torch.ones(*dirs.shape[:-1], self.cfg.n_output_dims).to(dirs.device))
        # print((torch.ones(*dirs.shape[:-1], self.cfg.n_output_dims).to(dirs.device)
        #     * self.env_color).shape)
        # print(dirs.device)
        # print(dirs.dtype)
        # print(torch.ones(1,device=dirs.device,dtype=dirs.dtype))
        color = (
