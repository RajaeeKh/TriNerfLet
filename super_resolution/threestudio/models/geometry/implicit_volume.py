...
class ImplicitVolume(BaseImplicitGeometry):
    ...
    def configure(self) -> None:
        ...
        if self.cfg.multiscale_kplsnes:
            assert self.cfg.pos_encoding_config.otype == 'multiscale_k_planes'
            self.feature_network_high_res = get_mlp(
                self.encoding.encoding.n_output_dims_high_res,
                self.cfg.n_feature_dims + 1,
                self.cfg.mlp_network_config,
            )
    ...
    def get_features(self,enc):
        # print(self.cfg.multiscale_kplsnes)
        # print(self.encoding.encoding.double_mode)
        # print(self.encoding.encoding.resolution_mode)
        if (self.cfg.multiscale_kplsnes and self.encoding.encoding.double_mode and
                (self.encoding.encoding.resolution_mode == 'high_res')):
            features = self.feature_network_high_res(enc)
            return features
        features = self.feature_network(enc)
        return features
    def density_network(self,enc):
        features = self.get_features(enc)
        density,features = self.split_density_features(features)
        return density

    def split_density_features(self,features):
    ...
    def forward(
        ...
        # print('grad_enabled = ',grad_enabled)
        enc = self.encoding(points.view(-1, self.cfg.n_input_dims))
        # density = self.density_network(enc).view(*points.shape[:-1], 1)

        features = self.get_features(enc).view(
            *points.shape[:-1], self.cfg.n_feature_dims + 1
        )
        density,features = self.split_density_features(features)
        ...
        if isinstance(other, ImplicitVolume):
            ...
            if copy_net:
                if (
                    ...
                    if instance.cfg.multiscale_kplsnes:
                        instance.feature_network_high_res.load_state_dict(
                            other.feature_network_high_res.state_dict()
                        )
                if (
