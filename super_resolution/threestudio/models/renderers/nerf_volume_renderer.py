...
class NeRFVolumeRenderer(VolumeRenderer):
    ...
    class Config(VolumeRenderer.Config):
        ...
        occgrid_resolution: int = 32
        occgrid_levels: int = 1
        render_step_factor: float = 1.0
    ...
    def configure(
        ...
        if self.cfg.estimator == "occgrid":
            self.estimator = nerfacc.OccGridEstimator(
                roi_aabb=self.bbox.view(-1), resolution=self.cfg.occgrid_resolution, levels=self.cfg.occgrid_levels
            )
            ...
            self.render_step_size *= self.cfg.render_step_factor
        ...
        print('************ prune_alpha_threshold: {} ******************'.format(self.cfg.prune_alpha_threshold))
    ...
    def forward(
        ...
        **kwargs
        ...
        # print('rays_o.shape2 = {}'.format(rays_o.shape))
        # print('rays_o_flatten.shape2 = {}'.format(rays_o_flatten.shape))
        # print('light_positions_flatten.shape2 = {}'.format(light_positions_flatten.shape))
        if self.cfg.estimator == "occgrid":
            if not self.cfg.grid_prune:
                with torch.no_grad():
                ...
                with torch.no_grad():
                    # print('rays_o_flatten.dtype ',rays_o_flatten.dtype)
                    ray_indices, t_starts_, t_ends_ = self.estimator.sampling(
                        rays_o_flatten,
                        rays_d_flatten,
        ...
        # print('t_starts_: {}'.format(t_starts_))
        # print('t_ends_: {}'.format(t_ends_))
        ...
        if self.cfg.estimator == "occgrid":
            if self.cfg.grid_prune:
                ...
                def occ_eval_fn(x):
                    ...
                    # print('update x.shape = {}'.format(x.shape))
                    # print(density)
                    # print(1 - torch.exp(-density * self.render_step_size))
                    return 1 - torch.exp(-density * self.render_step_size)
